...
The information in this section describes a very simple interaction with Edge.
To follow along with the steps in this section, use the default Edge DS tenant "default" and the default namespace "data".  
...
In this example you must trust the default certificate of Edge, which is temporary and is generated by OSIsoft. 
...
Each Edge DS ships with a default authentication token. The token is used by OMF applications to gain access to the Edge DS. 
You can view the token can in the Edge DS Admin UI. <https://localhost:5000> 
...
When working outside of .NET,  Qi libraries are unavailable. The QiType is defined using JSON 
and is posted to the OSIsoft Cloud Services endpoint.
