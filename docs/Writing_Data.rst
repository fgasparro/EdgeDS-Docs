...
Verbose JSON responses include all values, including defaults, in the returned JSON payload. 
To specify verbose JSON return, add the header Accept-Verbosity with a value of ``verbose`` to 
the request, and set the ``Accept`` header with the value: ``application/json``.
